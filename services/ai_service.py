...
class AIService:
    ...
    def chat(self, user_message: str, conversation_history: List[Dict] = None, mode: str = "conversation", user_profile: Dict = None, language_config: Dict = None) -> str:
        """
        ...
        teacher_name = language_config.get("teacherName", "Ms. Smile")
        language_name = language_config.get("languageName", "English")
        response_header = language_config.get("responseHeader", f"🇺🇸 {language_name}")
        bilingual_note = language_config.get("bilingualNote", "Bắt buộc trả lời song ngữ Anh - Việt.")

        # Xây dựng system prompt với user context
        system_prompt = language_config.get("systemPrompt") or SYSTEM_PROMPT
        
        # Bilingual output contract, driven by current language config.
        bilingual_format = f"""

BAT BUOC - FORMAT TRA LOI SONG NGU:

AI luon tra loi theo format sau:

{response_header}:
<cau {language_name} tu nhien, ngan, phu hop hoi thoai>
...
QUY TAC:
- Nếu user viết tiếng Việt: chuyển ý sang {language_name} tự nhiên, rồi giải thích bằng tiếng Việt.
- Nếu user viết {language_name} sai: sửa nhẹ, giải thích lỗi bằng tiếng Việt.
- Nếu user viết {language_name} đúng: khen cụ thể, giải thích từ vựng và cấu trúc.
- Luôn dùng giọng {teacher_name} thân thiện, dễ hiểu.
- {bilingual_note}
"""
...
- Vui lòng kiểm tra API key hoặc thử lại sau."""
        ...
        has_english = f'{response_header}:' in ai_response or 'US English:' in ai_response or '🇺🇸 English:' in ai_response
...
"""
    ...
    def _call_gemini(self, messages: List[Dict]) -> str:
        ...
        try:
            ...
            system_content = next((msg.get("content", "") for msg in messages if msg.get("role") == "system"), SYSTEM_PROMPT)
            prompt_parts.append(system_content or SYSTEM_PROMPT)
            prompt_parts.append("\n---\n")
            ...
            for msg in messages:
                ...
                if role == "system":
                    ...
                    prompt_parts.append(f"AI Teacher: {content}")
            
            # Thêm yêu cầu format phản hồi
            prompt_parts.append("\nHãy phản hồi đúng theo system prompt và format song ngữ:")
    ...
    def generate_lesson(self, level: str = "beginner", profile: Dict[str, Any] = None) -> Dict[str, Any]:
        """
